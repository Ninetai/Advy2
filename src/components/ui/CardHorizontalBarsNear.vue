    ...
    <div class="card" :class="cls">
        ...
        <div v-for="d, index in data" :key="index">
            <div class="d-flex mb-3">
                <div class="text-left mr-2" style="width: 100px">
                    {{ d[name] }}
                </div>
                <div class="d-flex align-items-center gap-2 flex-grow-1">
                    <div class="prog-bar" :class="minProg(d[value] * parseInt(mult))"
                        :style="{ width: Math.min((d[value] * 1.4 * parseInt(mult)), 100) + '%' }"></div>
                    <span style="font-weight:600">{{ Math.round(d[value] * parseInt(mult)) }}%</span>
                </div>
            </div>
        </div>
    </div>
